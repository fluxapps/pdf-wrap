...
import { LoadingOptions, PageRenderingMode, PDFDocumentService, ViewFeatures } from "../api";
import { Outline, PageThumbnail, PDFDocument, ScalePreset, TreeOutlineEntry } from "../api/document";
import { Circle, Ellipse, Line, Point, PolyLine, Rectangle } from "../api/draw";
import { PageChangeEvent, StateChangeEvent } from "../api/event";
import { Highlighting } from "../api/highlight";
import { DocumentSearch } from "../api/search";
import { PageEventCollection, StorageRegistry } from "../api/storage";
import { FormsTool, Toolbox } from "../api/tool";
import { ZoomSettings } from "../api/zoom";
...
export class PDFjsDocumentService implements PDFDocumentService {
    ...
    private async _loadWith(options: LoadingOptions): Promise<PDFDocument> {
        ...
        const forms: FormsTool = new FormFactory(documentModel, rescaleManager);
